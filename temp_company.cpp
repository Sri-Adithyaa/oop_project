...
class Company;
void update_file(Company &c1, string old_symbol);
...
public:
    ...
    friend void update_file(Company &c1, string old_symbol);
    ...
    void Release_stock(int stock)
    {
        no_shares+=stock;
        string temp_store;
        temp_store="sync_engine.exe C_SHARE "+to_string(no_shares);
    ...
    void update_data()
    {
        ...
        switch(ch)
        {
            ...
            {
                string temp_store;
                L3:cout<<"Enter the new name of the company = ";
                cin>>temp_store;
                ...
                {
                    string old_symbol = symbol; // important
                    string o_name=name;
                    name=temp_store;

                    string cmd="sync_engine.exe C_NAME "+symbol+" "+name+" "+o_name;
                    update_file(*this, old_symbol);
                    system(cmd.c_str());
                }
                break;
            }
            ...
            case 2:
            {
                ...
                {
                    string old_symbol = symbol;  // VERY IMPORTANT
                    string o_sym = symbol;

                    symbol=temp_store;

                    string cmd="sync_engine.exe C_SYM "+symbol+" "+o_sym;
                    update_file(*this, old_symbol);
                    system(cmd.c_str());
                }
                break;
...
void update_file(Company &c1, string old_symbol)
{
    ...
    while(getline(f1,line))
    {
        if(line.empty()) continue; // avoid crash
...
{
    os<<c.symbol<<"|"<<fixed<<setprecision(2)<<c.net_worth<<"|"
      <<c.no_shares<<"|"<<fixed<<setprecision(2)<<c.ipo<<"|"<<c.name;
    return os;
}
...
int main(int argc,char* argv[])
{
    ...
    else
    {
        ...
        fstream f1("admin_user.txt",ios::in);
        cout<<"Enter the admin user name and password to add a new company"<<endl;
        L2:cout<<"Enter the admin User name= ";
        cin>>admin;
